...
TRUNCATE TABLE IF EXISTS sakila_bi.customer_dim;
INSERT INTO sakila_bi.customer_dim (
	customer_id,
	first_name,
	last_name,
	email,
...
TRUNCATE TABLE IF EXISTS sakila_bi.staff_dim;
INSERT INTO sakila_bi.staff_dim (
	staff_id,
	first_name,
	last_name,
	address,
...
TRUNCATE TABLE IF EXISTS sakila_bi.store_dim;
INSERT INTO sakila_bi.store_dim (
	store_id,
	manager_firstname,
	manager_lastname,
	address,
...
TRUNCATE TABLE IF EXISTS sakila_bi.film_dim;
INSERT INTO sakila_bi.film_dim (
	film_id,
	title,
	description,
	released_year,
...
CREATE OR REPLACE TRANSIENT TABLE sakila_bi.trans_base_stg AS  
SELECT
...
CREATE OR REPLACE TRANSIENT TABLE sakila_bi.last_4_wk_stg AS 
SELECT 
	c.cal_dt,
	c.yr_wk_num
FROM sakila.calendar_dim c
JOIN 
		(SELECT 
			yr_wk_num
...
CREATE OR REPLACE TRANSIENT TABLE sakila_bi.last_4_wk_store_stg AS 
SELECT 
	w.yr_wk_num,
	s.store_id,
	w.cal_dt
FROM sakila_bi.last_4_wk_stg w
CROSS JOIN sakila_bi.store_dim s;



----------- filter out the sum transaction with the latest 4 weeks
CREATE OR REPLACE TRANSIENT TABLE sakila_bi.last_4_wk_trans_stg AS 
SELECT 
	w.store_id,
	w.yr_wk_num,
	nvl(sum(t.amount),0) AS wk_amount
FROM 
sakila_bi.last_4_wk_store_stg w 
LEFT JOIN sakila_bi.trans_base_stg T  ON T.trans_dt=w.cal_dt AND T.STORE_ID=w.STORE_ID 
GROUP BY 1,2
ORDER BY 1,2;
...
CREATE OR REPLACE TRANSIENT TABLE sakila_bi.last_4_wk_decline AS 
SELECT 
		store_id,
		             ...
		             (SELECT *, CASE WHEN wk_amount - last_wk_amount<=0 THEN -1 ELSE 0 END AS wk_decline
				       FROM  
				        		(select  *, lead(wk_amount) over (partition by store_id order by yr_wk_num) as last_wk_amount 
				        		 from sakila_bi.last_4_wk_trans_stg)
				       WHERE last_wk_amount IS NOT NULL)
		  group by 1)
;

------- finally we join this column with the sakila_bi.trans_base_stg table, to create the final transcaction transient table.
CREATE OR REPLACE TRANSIENT TABLE sakila_bi.transaction_stg AS 
SELECT T.*,
			  w.is_decline
FROM sakila_bi.trans_base_stg t
JOIN sakila_bi.last_4_wk_decline w USING (store_id);

--------- replace the current transaction table with new transaction transient table
TRUNCATE TABLE IF EXISTS sakila_bi.transaction;
INSERT INTO sakila_bi.transaction (
		trans_dt,
		customer_id,
		staff_id,
		store_id,
...
SELECT 
		...
		amount,
		is_decline
FROM sakila_bi.transaction_stg;
